...
# Find SSH key (auto-generated by Terraform)
SSH_KEY=""
DEFAULT_KEY="$TERRAFORM_DIR/moni-dev-k3s-key.pem"
if [ -f "$DEFAULT_KEY" ]; then
    SSH_KEY="$DEFAULT_KEY"
else
    PEM_FILE=$(find "$TERRAFORM_DIR" -maxdepth 1 -name "*.pem" 2>/dev/null | head -1)
    [ -n "$PEM_FILE" ] && SSH_KEY="$PEM_FILE"
fi
...
ssh_exec() {
    if [ -z "$SSH_KEY" ]; then
        echo -e "${RED}ERROR: SSH key not found in $TERRAFORM_DIR${NC}"
        echo "Run 'terraform apply' to generate the key, or place it at $DEFAULT_KEY"
        exit 1
    fi
...
show_help() {
    ...
    echo ""
    echo "SSH / K3s system commands (run directly on EC2):"
    echo "  ssh         - Open SSH session on the EC2 instance"
    echo "  k3s-status  - Show K3s systemd service status"
    echo "  k3s-logs    - Show K3s system logs (last 50 lines)"
    echo "  k3s-logs-f  - Follow K3s system logs live"
    echo "  boot-logs   - Show EC2 user-data/boot logs (/var/log/user-data.log)"
    echo ""
}
...
cmd_ssh() {
    echo -e "${YELLOW}Connecting to ec2-user@$EC2_IP...${NC}"
    ssh $SSH_OPTS -i "$SSH_KEY" "ec2-user@$EC2_IP"
}

cmd_k3s_status() {
    echo -e "${YELLOW}K3s service status on $EC2_IP:${NC}"
    ssh_exec "systemctl status k3s --no-pager"
}

cmd_k3s_logs() {
    echo -e "${YELLOW}K3s system logs (last 50 lines) from $EC2_IP:${NC}"
    ssh_exec "journalctl -u k3s -n 50 --no-pager"
}

cmd_k3s_logs_follow() {
    echo -e "${YELLOW}Following K3s system logs on $EC2_IP... (Ctrl+C to exit)${NC}"
    ssh_exec "journalctl -u k3s -f"
}

cmd_boot_logs() {
    echo -e "${YELLOW}EC2 user-data/boot logs from $EC2_IP:${NC}"
    ssh_exec "cat /var/log/user-data.log"
}
