...
resource "aws_route_table" "public" {
  ...
  tags = merge(local.common_tags, {
    Name = "${local.instance_name}-public-rt"
  })
}
...
resource "aws_security_group" "k3s" {
  name_prefix = "${local.instance_name}-k3s-"
  vpc_id      = aws_vpc.main.id
  ...
  # K3s API server (for kubectl access)
  ingress {
    description = "K3s API"
    from_port   = 6443
    to_port     = 6443
    protocol    = "tcp"
    cidr_blocks = var.allowed_ssh_cidr  # Same as SSH for security
  }
